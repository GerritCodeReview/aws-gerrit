...
git config -f /var/gerrit/etc/gerrit.config gerrit.serverId "0f56469f-dfe4-4f28-aca7-803eba7845e0"
...
if [ $CONTAINER_REPLICA ]; then
  ...
  if [ ! -d /var/gerrit/git/All-Projects.git ] ||
     ...
     aws s3 cp s3://gerritforge-git-bootstrap/All-Users.git.tar.gz /tmp
     tar xzvf /tmp/All-Users.git.tar.gz -C /var/gerrit/git/

     echo "[REPLICA] Installing All-Projects from S3..."
     aws s3 cp s3://gerritforge-git-bootstrap/All-Projects.git.tar.gz /tmp
