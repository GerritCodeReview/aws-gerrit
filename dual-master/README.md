...
* `MULTISITE_REPLICATION_TARGET_URL`: Optional.  The fully qualified domain name of an additional replication target.
Empty by default.

The replication service and the multi-site replication target represent the reading
and writing sides of Git replication: by enabling both of them, it is possible to
establish replication between two different gerrit sites.
